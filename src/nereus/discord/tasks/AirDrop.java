...
public class AirDrop implements Runnable
{
    ...
    public void run()
    {
        try {
            ...
            world.db.jdbc.run("INSERT INTO redeems (Code, Enabled, Limited, Quantity, QuantityLeft, ItemID, Gold, Coins, Exp, ClassPoints, DateExpiry) VALUES (?, 1, 1, ?, 1, ?, 0, 0, 0, 0, NOW())", code, quantity, item.getId());
...
//            **[" + world.clearHTMLTags(item.getName()) + "](" + ConfigData.SERVER_PROFILE_LINK + item.getId() + ") x" + Quantity + "**
            third.setDescription("Congratulation <@" + user.getId() + ">! You won **[" + world.clearHTMLTags(item.getName()) + "](" + ConfigData.SERVER_PROFILE_LINK + item.getId() + ") x" + quantity + "** from discord air box event! Your redeem code is: **" + code + "**.");
