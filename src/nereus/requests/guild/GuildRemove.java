...
public class GuildRemove implements IRequest {
   ...
   public void process(String[] params, User user, World world, Room room) throws RequestException {
      ...
      QueryResult result = world.db.jdbc.query("SELECT users.id, users.username, users_guilds.GuildID, users_guilds.Rank FROM users LEFT JOIN users_guilds ON UserID = id WHERE username = ?", new Object[]{params[1]});
      if(!result.next()) {
         result.close();
