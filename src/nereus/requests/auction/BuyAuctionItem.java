    ...
    public void process(String[] params, User user, World world, Room room) throws RequestException {
        ...
        try {
            ...
            if (auctionResult.next()) {
                ...
                int inventoryCount = world.db.jdbc.queryForInt("SELECT COUNT(*) AS rowcount FROM users_items LEFT JOIN items ON items.id = users_items.ItemID WHERE Equipment NOT IN ('ho','hi') AND Bank = 0 AND UserID = ? AND ItemID NOT IN (?)", user.properties.get(Users.DATABASE_ID), item.getId());
                int sellerAccess = world.db.jdbc.queryForInt("SELECT Access FROM users WHERE id = ?", auctionResult.getInt("UserID"));
                if (item.isUpgrade() && (Integer) user.properties.get(Users.UPGRADE_DAYS) < 0) {
                    ...
                    QueryResult userResult = world.db.jdbc.query("SELECT Gold, Coins FROM users WHERE id = ? FOR UPDATE", (Integer) user.properties.get(Users.DATABASE_ID));
                    if (userResult.next()) {
                        ...
                        if (gold >= auctionResult.getInt("Gold") && coins >= auctionResult.getInt("Coins")) {
                            QueryResult itemResult = world.db.jdbc.query("SELECT id FROM users_items WHERE ItemID = ? AND UserID = ? AND Bank = 0", item.getId(), (Integer) user.properties.get(Users.DATABASE_ID));
                            ...
                            if (itemResult.next()) {
                                ...
                                if (item.getStack() > 1) {
                                    ...
                                    world.db.jdbc.run("INSERT INTO users_markets_logs (OwnerID, BuyerID, Coins, Gold, ItemID, EnhID, Quantity, Type) VALUES (?, ?, ?, ?, ?, ?, ?, 'Buy')", SellerID, (Integer) user.properties.get(Users.DATABASE_ID), auctionResult.getInt("Coins"), auctionResult.getInt("Gold"), item.getId(), auctionResult.getInt("EnhID"), auctionResult.getInt("Quantity"));
                                ...
                                itemResult.close();
                                world.db.jdbc.run("INSERT INTO users_markets_logs (OwnerID, BuyerID, Coins, Gold, ItemID, EnhID, Quantity, Type) VALUES (?, ?, ?, ?, ?, ?, ?, 'Buy')", SellerID, (Integer) user.properties.get(Users.DATABASE_ID), auctionResult.getInt("Coins"), auctionResult.getInt("Gold"), item.getId(), auctionResult.getInt("EnhID"), auctionResult.getInt("Quantity"));
                                world.db.jdbc.run("INSERT INTO users_items (UserID, ItemID, EnhID, Equipped, Quantity, Bank) VALUES (?, ?, ?, 0, ?, 0)", (Integer) user.properties.get(Users.DATABASE_ID), item.getId(), auctionResult.getInt("EnhID"), auctionResult.getInt("Quantity"));
                                charItemId = Long.valueOf(world.db.jdbc.getLastInsertId()).intValue();
                            }

                            if (charItemId > 0) {
                                world.db.jdbc.run("UPDATE users SET Gold = ?, Coins = ? WHERE id = ?", gold - auctionResult.getInt("Gold"), coins - auctionResult.getInt("Coins"), user.properties.get(Users.DATABASE_ID));
                                world.db.jdbc.run("UPDATE users_markets SET BuyerID = ?, Status = 0 WHERE id = ?", user.properties.get(Users.DATABASE_ID), auctionId);
