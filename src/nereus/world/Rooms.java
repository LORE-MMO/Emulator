...
import nereus.db.objects.Hall;
import nereus.db.objects.House;
import nereus.db.objects.MapMonster;
import nereus.db.objects.Monster;
import nereus.tasks.WarpUser;
...
public class Rooms {
   ...
   private void moveToArea(Room room, User user) {
      ...
      mta.put("strMapBackgroundMusic", area.getMusic());
      mta.put("strMapBackgroundMusicStrength", area.getMusicStrength());
   ...
   public void basicRoomJoin(User user, String roomName, String roomFrame, String roomPad) {
      ...
      Room roomToJoin = lookForRoom(roomName, user);

      if (roomToJoin == null)
         roomToJoin = generateRoom(roomName, user);
   ...
   public Room lookForRoom(String name, User user) {
   ...
   public Room generateRoom(String name, User user) {
      Area area = this.world.areas.get(name.split("-")[0]);

      if (name.contains("-"))
         try {
            if (StringUtils.isNumeric(name.split("-")[1])) {
               int roomKey = Integer.parseInt(name.split("-")[1]);
               if (roomKey > 9999) {
                  String generatedName = name.split("-")[0] + "-" + (this.privKeyGenerator.nextInt(9999));
               ...
               this.world.send(new String[]{"warning", "You cannot access other guild rooms!"}, user);
            }
         } catch (NumberFormatException nre) {
